...
with rej as(insert into foo(a, b, c, d, e, f, g, merge, payload) values (:reca, :recb, :recc, :recd, :rece, :recf, :recg, :recmerge, 'insert'), (:sreca, :srecb, :srecc, :srecd, :srece, :srecf, :srecg, :srecmerge, 'insert') on duplicate key lock for update returning rejects ctid, *) update foo set payload = (random() * 5)::text from rej where foo.ctid = rej.ctid;
